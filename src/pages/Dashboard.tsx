...
const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { value: number; name: string }[]; label?: string }) => {
  ...
  return (
    <div className="text-xs px-3 py-2 rounded-xl" style={{ background: '#1E1639', border: '1px solid #2A1F4A', color: '#F5F3FF' }}>
      <p className="font-semibold mb-1">{label}</p>
      {payload.map(p => (
        <p key={p.name} style={{ color: p.name === 'paid' ? '#6EE7B7' : '#A78BFA' }}>
